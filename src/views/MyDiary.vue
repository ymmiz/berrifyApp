            ...
            <div class="entry-content">
              ...
              <div class="mode-badge" :class="plant.mode">
                <i
                  :class="
                    plant.trackingIcon ||
                    (plant.mode === 'hardware' ? 'bi bi-cpu' : 'bi bi-upload')
                  "
                ></i>
                {{
                  plant.mode === "hardware"
              ...
              <p class="tracking-description">
                ...
                {{
                  ...
                  (plant.mode === "hardware"
                    ? "Tracking: Hardware Device - Real-time monitoring"
                    : "Tracking: Photo Analysis - Upload photos for analysis")
                }}
              </p>
              <p>
                {{
              ...
              <p class="status-alert">
                {{
                  ...
                  (plant.mode === "hardware"
                    ...
                    : "Upload strawberry photos to get analysis")
                }}
              </p>
              ...
              <div class="plant-actions">
                ...
                <router-link
                  ...
                  :to="{
                    ...
                    query: { plantId: plant.id, plantName: plant.name },
                  }"
                >
                ...
                <button
                  ...
                  :class="{
                    ...
                    'water-btn--blocked':
                      !plant.hasPhoto && !plant.wateredToday,
                  }"
                  :title="
                    !plant.hasPhoto && !plant.wateredToday
                      ? 'Add/scan a photo first'
                      : ''
                  "
      ...
      <div v-if="showChooseMode" class="modal-overlay" @click="closeChooseMode">
        <div class="choose-mode-modal" @click.stop>
          ...
          <div
            ...
            class="mode-badge"
            :class="selectedMode"
            title="Tracking mode is fixed"
          >
            <i
              :class="
                selectedMode === 'hardware' ? 'bi bi-cpu' : 'bi bi-upload'
              "
            ></i>
            {{
...
export default {
    ...
    allPlants() {
      ...
      return this.plants
        ...
        .map((plant) => {
          ...
          const hasPhoto = !!(
            plant.photo_count > 0 ||
            plant.last_photo_url ||
            plant.latestPhotoPath ||
            plant.firstPhotoAt
          );
  ...
  methods: {
    ...
    async removePlant(plantId) {
      ...
      if (
        !confirm(
          `Are you sure you want to remove "${plant.name}"? This will delete all associated data including diary entries and uploads.`
        )
      )
        return;
...
// In the methods section, replace the existing viewPlantPhotos method:
    ...
    goToToday() {
      ...
      this.$nextTick(() => {
        ...
        try {
          ...
          if (vuecal?.updateSelectedDate)
            return vuecal.updateSelectedDate(today);
    ...
    async markAsWatered(plant) {
      ...
      if (!ok) {
        alert(
          "Please add or scan a photo of this plant before marking as watered."
        );
        return;
      }
    ...
    async startWateringCheck() {
      ...
      if (this._wateringTimer) clearInterval(this._wateringTimer);
      this._wateringTimer = setInterval(
        () => this.checkWateringSchedule({ force: false }),
        3600000
      ); // hourly
    },
    ...
    async checkWateringSchedule(opts = {}) {
      try {
        ...
        const notWatered = this.allPlants.filter((plant) => {
          return !this.isWateredToday(
            plant.lastWateredAt || plant.last_watered,
            tz
          );
        });
        ...
        const extra =
          notWatered.length > 1 ? ` and ${notWatered.length - 1} more` : "";
    ...
    async debugPlantData(plantName = null) {
      const plant = plantName
        ? this.plants.find((p) => p.name === plantName)
        : this.plants[0];
      ...
      console.log("Plant debug data:", {
        ...
        wateredToday: this.isWateredToday(
          plant.lastWateredAt || plant.last_watered
        ),
